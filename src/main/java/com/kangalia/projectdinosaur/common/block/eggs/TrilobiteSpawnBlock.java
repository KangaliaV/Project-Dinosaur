...
public class TrilobiteSpawnBlock extends WaterSpawnBlock {

    public TrilobiteSpawnBlock(Properties pProperties, Supplier<? extends EntityType<? extends PrehistoricEntity>> entity) {
        ...
        for(int j = 1; j <= i; ++j) {
            ...
            if (young instanceof TrilobiteEntity) {
                ...
                if (Objects.equals(parent1Genes, "") || Objects.equals(parent2Genes, "") || parent1Genes == null || parent2Genes == null) {
                    ((TrilobiteEntity) young).setGenes(((TrilobiteEntity) young).generateGenes(false));
                } else {
                    ((TrilobiteEntity) young).setGenes(((TrilobiteEntity) young).inheritGenes(parent1Genes, parent2Genes));
                }
    ...
    public String getParent1GenesFromSpawnBlockEntity(BlockEntity entity) {
        if (entity instanceof TrilobiteSpawnBlockEntity) {
            return ((TrilobiteSpawnBlockEntity) entity).getParent1();
        } else {
            return "";
        }
    }
    ...
    public String getParent2GenesFromSpawnBlockEntity(BlockEntity entity) {
        if (entity instanceof TrilobiteSpawnBlockEntity) {
            return ((TrilobiteSpawnBlockEntity) entity).getParent2();
        } else {
            return "";
        }
    }
    ...
    public void setPlacedBy(Level pLevel, BlockPos pPos, BlockState pState, @org.jetbrains.annotations.Nullable LivingEntity pPlacer, ItemStack pStack) {
        ...
        if (!pLevel.isClientSide) {
            if (entity instanceof TrilobiteSpawnBlockEntity eggEntity) {
    ...
    public void playerWillDestroy(Level pLevel, BlockPos pPos, BlockState pState, Player pPlayer) {
        ...
        if (!pLevel.isClientSide && !pPlayer.isCreative()) {
            if (blockentity instanceof TrilobiteSpawnBlockEntity eggBlockEntity) {
    ...
    public BlockEntity newBlockEntity(BlockPos pPos, BlockState pState) {
        return BlockEntitiesInit.TRILOBITE_SPAWN_ENTITY.get().create(pPos, pState);
    }
}
